 ...
 */
function catchFunctionError<T, D extends unknown[]>(callback: (...args: D) => T): (...args: D) => T | null {
  return (...args: D): T | null => {
    try {
      return callback(...args);
    } catch (error) {
      const message = error instanceof Error ? `Ошибка. ${error.stack || ''}` : 'Неизвестная клиентская ошибка.'
      Notify.create({
        message,
        color: 'negative'
      });
